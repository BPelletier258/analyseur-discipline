# === CANVAS META =============================================================
# Fichier : main.py — version (09-09) surlignage + dates + tirets (aperçu)
# ============================================================================
...
def build_article_pattern(user_input: str) -> re.Pattern:
    ...
    #      └─ CAPTURE (\\1) = le numéro d'article → on s’en sert pour <span class="hit">\\1</span>
    pattern = rf"(?:\\b(?:art(?:icle)?\\s*[: ]*)?)([{esc}]{''}){tail}".replace("[{esc}]{''}", esc)
...
# ──────────────────────────────────────────────────────────────────────────────
# Extraction nettoyée (mode “segment uniquement” par défaut)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Surlignage des occurrences et formatage des dates (pour l’APERÇU HTML UNIQUEMENT)
# ──────────────────────────────────────────────────────────────────────────────

def highlight_for_html(df: pd.DataFrame, colmap: Dict[str, Optional[str]], pat: re.Pattern) -> pd.DataFrame:
    """
    Enveloppe le numéro d’article trouvé par <span class="hit">…</span> dans les colonnes d’intérêt.
    Ne modifie PAS le df original utilisé pour Excel : on travaille sur une copie dédiée à l’aperçu.
    """
    html_df = df.copy()

    def _hl(val: object) -> object:
        if not isinstance(val, str) or not val:
            return val
        return pat.sub(r'<span class="hit">\\1</span>', val)

    targets = [colmap.get(c) for c in FILTER_CANONICAL]
    for col in targets:
        if col and col in html_df.columns:
            html_df[col] = html_df[col].astype(str).apply(_hl)

    # Beaucoup de fichiers utilisent aussi “Liste des chefs et articles en infraction”
    # → si elle existe sous son libellé exact, on la surligne aussi.
    for name in html_df.columns:
        if _norm(name) == _norm("Liste des chefs et articles en infraction"):
            html_df[name] = html_df[name].astype(str).apply(_hl)

    return html_df

def format_dates_for_html(df: pd.DataFrame) -> pd.DataFrame:
    """
    Rend les dates visibles dans l’aperçu : yyyy-mm-dd HH:MM:SS.
    Valeurs manquantes → “—” (sans puce).
    """
    html_df = df.copy()

    def is_target(col: str) -> bool:
        n = _norm(col)
        return n in {_norm("Date de création"), _norm("Date de mise à jour"),
                     _norm("Date de creation"), _norm("Date de mise a jour")}

    for col in html_df.columns:
        if is_target(col):
            ser = pd.to_datetime(html_df[col], errors="coerce")
            html_df[col] = ser.dt.strftime("%Y-%m-%d %H:%M:%S")
            html_df[col] = html_df[col].fillna("—")

    # Affichage plus propre des “nan” qui restent dans d’autres colonnes
...
def analyze():
    ...
    if not (fname.endswith(".xlsx") or fname.endswith(".xlsm")):
        return render_template_string(
            ...
            table_html=None,
            searched_article=article,
            message=("Format non pris en charge. Fournir un .xlsx ou .xlsm."),
            message_ok=False
        )
    ...
    try:
        ...
        # Filtrage (au moins une des 4 colonnes contient l’article)
        masks = []
        any_cols = False
        for canon in FILTER_CANONICAL:
            col = colmap.get(canon)
        ...
        # Épuration des cellules pour ne garder que le segment qui contient l’article
        ...
        # -------- Excel (données brutes propres) ----------
        download_url = to_excel_download(df_clean)

        # -------- Aperçu HTML : dates formatées + surlignage + tirets ----------
        df_view = format_dates_for_html(df_clean)
        df_view = highlight_for_html(df_view, colmap, pat)

        # Aperçu limité à 200 lignes
        ...
        return render_template_string(
            ...
            message=f"{len(df_clean)} ligne(s) après filtrage. (Aperçu limité à 200 lignes.)",
            message_ok=True
        )
