...
# CONFIGURATION
# -----------------------------------------------------------------------------
target_article = r"Art[.:]\s*14(?=\D|$)"
output_file = "decisions_article_14_formate.xlsx"
app = Flask(__name__)

# simple HTML template with upload form
INDEX_HTML = '''
<!doctype html>
...
def analyze():
    ...
    try:
        filtered = filter_dataframe(file)
    except Exception as e:
        abort(400, str(e))
    # extract urls separately if present
    file.stream.seek(0)
    df_all = pd.read_excel(file.stream)
    urls = df_all.get('resume', ['']*len(df_all)).tolist()
    # Excel
    bio = build_excel(filtered.assign(Résumé=['Résumé']*len(filtered)), urls)
    # Prepare Markdown
