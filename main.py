# === CANVAS META =============================================================
# Fichier : main.py — version + motif corrigé + bullets + highlight (09-08 pm)
# ============================================================================
# - Normalisation des en-têtes (accents, casse, espaces insécables)
# - Aliases corrigés (pas de confusion entre "Nbr Chefs par articles" et
...
# - Rendu HTML : listes à puces + surlignage de l’article (rouge)
...
<style>
  ...
  /* Zone scrollable horizontale et verticale */
  .table-viewport{height:60vh;overflow:auto;border:1px solid var(--border)}
  /* Largeur min ~ 2 écrans ; augmente selon contenu */
  .table-wide{min-width:200vw}
  .table-viewport table{width:100%}

  /* Listes à puces propres dans les cellules */
  td ul{margin:0 0 0 1.1em;padding:0}
  td li{margin:0 2px 0 0}
...
def _norm(s: str) -> str:
    ...
    s = s.replace("\u00A0", " ")  # insécables
    s = " ".join(s.strip().lower().split())
    return s

# Colonnes canoniques utilisées par l’app
# NB : on sépare explicitement
#   - « Nbr Chefs par articles » (canon: articles_enfreints)
#   - « Liste des chefs et articles en infraction » (canon: liste_chefs_articles)
HEADER_ALIASES: Dict[str, Set[str]] = {
    # = « Nbr Chefs par articles » (colonne d’intérêt #1)
    "articles_enfreints": {
        _norm("Nbr Chefs par articles"),
        _norm("Nombre de chefs par articles"),
        _norm("Articles enfreints"),          # anciens libellés
        _norm("Articles en infraction"),
        _norm("Nbr   Chefs   par    articles"),
    },
    # = « Nbr Chefs par articles par période de radiation » (colonne d’intérêt #2)
    ...
    # = « Nombre de chefs par articles et total amendes » (colonne d’intérêt #3)
    "article_amende_chef": {
    ...
    # = « Nombre de chefs par article ayant une réprimande » (colonne d’intérêt #4)
    "autres_sanctions": {
    ...
    # = « Liste des chefs et articles en infraction » (affichage, non filtrant)
    "liste_chefs_articles": {
        _norm("Liste des chefs et articles en infraction"),
    },

    # Autres identifiants éventuels
    "numero_decision": {
...
# Colonnes canoniques utilisées pour le filtrage / nettoyage (les 4 d’intérêt)
FILTER_CANONICAL = [
    "articles_enfreints",
    "duree_totale_radiation",
    "article_amende_chef",
...
def build_article_pattern(user_input: str) -> re.Pattern:
    ...
    # Capture groupe(1) = le token pour pouvoir le surligner
    pattern = rf"(?:\\b(?:art(?:icle)?\\s*[: ]*)?)({esc}){tail}"
    return re.compile(pattern, flags=re.IGNORECASE)

# ──────────────────────────────────────────────────────────────────────────────
# Pré-traitement texte (conserve les retours à la ligne)
# ──────────────────────────────────────────────────────────────────────────────

def _prep_text(v: str) -> str:
    """Nettoie sans supprimer les retours à la ligne."""
    if not isinstance(v, str):
        v = "" if v is None else str(v)
    # retirer anciennes puces pour éviter la double-liste
    v = v.replace("•", " ").replace("·", " ").replace("◦", " ")
    # normaliser NBSP → espace
    v = v.replace("\u00A0", " ").replace("\u202F", " ")
    # normaliser fins de ligne
    v = v.replace("\r\n", "\n").replace("\r", "\n")
    # la séquence littérale "\n" → vraie NL
    v = v.replace("\\n", "\n")
    # compacter espaces SANS toucher aux \n
    v = re.sub(r"[ \t]+", " ", v).strip()
    return v

# ──────────────────────────────────────────────────────────────────────────────
# Extraction nettoyée dans les cellules (on ne garde que les segments où l’article apparaît)
# ──────────────────────────────────────────────────────────────────────────────
...
def clean_filtered_df(df: pd.DataFrame, colmap: Dict[str, Optional[str]], pat: re.Pattern) -> pd.DataFrame:
    ...
    # filtrage des lignes : on garde celles où AU MOINS une colonne d’intérêt contient l’article
...
# ──────────────────────────────────────────────────────────────────────────────
# Helpers : rendu HTML en puces + surlignage
# ──────────────────────────────────────────────────────────────────────────────

def _highlight_article_html(text_html: str, pat: re.Pattern) -> str:
    """Surligne le groupe(1) (le token) *dans du HTML déjà échappé*."""
    def repl(m: re.Match) -> str:
        g1 = m.group(1)
        if not g1:
            return m.group(0)
        return m.group(0).replace(g1, f'<span class="hl">{g1}</span>')
    return pat.sub(repl, text_html)

def _as_bullets_html(text: str, pat: re.Pattern, highlight: bool = True) -> str:
    """Transforme une chaîne en <ul><li>…</li></ul> en découpant sur |, ; ou \\n."""
    if not isinstance(text, str) or not text.strip():
        return ""
    ...
    if len(parts) <= 1:
        base = escape(text)
        return _highlight_article_html(base, pat) if highlight else base
    lis = []
    for p in parts:
        cell = escape(p)
        if highlight:
            cell = _highlight_article_html(cell, pat)
        lis.append(f"<li>{cell}</li>")
    return "<ul>" + "".join(lis) + "</ul>"

# ──────────────────────────────────────────────────────────────────────────────
# Export Excel (on exporte la version nettoyée texte, pas le HTML)
# ──────────────────────────────────────────────────────────────────────────────
...
def analyze():
    ...
    if not (fname.endswith(".xlsx") or fname.endswith(".xlsm")):
        return render_template_string(
            ...
            message=("Format non pris en charge : "
                     ...
                     + ". Veuillez fournir un .xlsx ou .xlsm."),
            message_ok=False
        )
    ...
    try:
        ...
        # Vérifier présence d'au moins une colonne d'intérêt
        masks = []
        any_cols = False
        for canon in FILTER_CANONICAL:
            col = colmap.get(canon)
        ...
        if not any_cols:
            ...
            return render_template_string(
                ...
                message=("Erreur : aucune des colonnes attendues n’a été trouvée dans le fichier.\n"
                         "Vérifiez les en-têtes ou ajoutez des alias.\n\n"
                         f"Colonnes résolues :\n{detail}\n\nColonnes disponibles :\n{list(df.columns)}"),
                message_ok=False
            )

        # Filtrage de lignes : on garde toute ligne où l'article apparait dans ≥1 des 4 colonnes d'intérêt
        mask_any = masks[0]
        for m in masks[1:]:
            mask_any = mask_any | m
        ...
        # Nettoyage des 4 colonnes (on ne garde que les segments contenant l'article)
        ...
        # Vue HTML : transformer en listes à puces + surligner l'article
        df_view = df_clean.copy()

        # Colonnes à rendre en puces + highlight (les 4 d’intérêt + « Liste des chefs et articles en infraction » si présente)
        bullet_cols = [colmap.get(k) for k in FILTER_CANONICAL if colmap.get(k)]
        if colmap.get("liste_chefs_articles"):
            bullet_cols.append(colmap["liste_chefs_articles"])

        for col in bullet_cols:
            if col in df_view.columns:
                df_view[col] = df_view[col].apply(lambda v: _as_bullets_html(_prep_text(v), pat, highlight=True))

        # Générer HTML (escape=False car contenu déjà échappé/contrôlé)
        preview = df_view.head(200)
        table_html = preview.to_html(index=False, escape=False)

        # Fichier Excel = version texte nettoyée (df_clean)
